...
class PoisonedDatasetFolder(DatasetFolder):
    def __init__(self,
        ...
        for idx, pattern in enumerate(patterns):    # 'patterns' is a list of tensors
            ...
            self.add_trigger_tranforms.append(AddDatasetFolderTrigger(pattern, weight))
    ...
    def __getitem__(self, index):
        ...
        if self.poisoned_rate >= 1.0:       # Poison testset
            ...
            if index in self.poisoned_set:
                idx = self.poisoned_set.index(index) % len(self.add_trigger_tranforms)
                poisoned_transform = copy.deepcopy(self.poisoned_transform)
                poisoned_transform.transforms.insert(self.poisoned_transform_index, self.add_trigger_tranforms[idx])
                sample = poisoned_transform(sample)
                # Change labels
                target = self.poisoned_target_transform(target)
            elif index in self.covered_set:
                idx = self.covered_set.index(index) % len(self.add_trigger_tranforms)
                poisoned_transform = copy.deepcopy(self.poisoned_transform)
                poisoned_transform.transforms.insert(self.poisoned_transform_index, self.add_trigger_tranforms[idx])
                sample = poisoned_transform(sample)
                # Remain labels
...
def CreatePoisonedDataset(benign_dataset, y_target, poisoned_rate, covered_rate, patterns, alphas, poisoned_transform_index, poisoned_target_transform_index):
...
class AdaptivePatch(Base):
    ...
    Args:
        ...
        covered_rate (float): Ratio of covered samples.
        patterns (None | torch.Tensor): Trigger patterns, shape (N, C, H, W). Default: N=4.
        weights (None | torch.Tensor): Weights of trigger patterns, shape (N, C, H, W). Default: N=4.
    ...
    def __init__(self,
                 ...
                 seed=0,
                 deterministic=False):
        # assert patterns is None or (isinstance(patterns, torch.Tensor) and ((0 < pattern) & (pattern < 1)).sum() == 0), 'pattern should be None or 0-1 torch.Tensor.'
